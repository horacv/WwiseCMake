...
namespace
{
    ...
    const std::string& mediaName_6 = "6_Streamed_WavFile.wav";
    ...
    std::vector<EventAndMediaInfo> eventsAndMedia = {
        ...
        {externalSourceSingle, mediaName_6  + " - Single - WAV File",
            {{mediaName_6, externalSourceSingle, AKCODECID_PCM_WAV, true}}},
    };
...
void PageExternalSources::ExternalSourceEventCallback(const AudioCallbackType type,
    AudioEventCallbackInfo* eventInfo, void* callbackInfo, void* cookie)
{
    if (!eventInfo) { return; }
    const auto page = static_cast<PageExternalSources*>(cookie);
    if (!page) { return; }

    if (type == AK_EndOfEvent)
    {
        page->HandleClearUnusedResources(eventInfo->playingID);
    }
}
