...
def copy_api_files(temp_dir, platform):
    """Copy SDL_ttf files from temp directory to project."""
    ...
    dst = SDL_TTF_LIB_DIR / "lib" / config['lib_subdir']
    if src.exists():
        dst.parent.mkdir(parents=True, exist_ok=True)
        if src.exists():
            if platform == "mac":
                # Copy the whole framework bundle into libs/sdl/lib/mac/SDL3_ttf.framework
                dst = SDL_TTF_LIB_DIR / "lib" / config['lib_subdir'] / src.name
                dst.parent.mkdir(parents=True, exist_ok=True)
                shutil.copytree(src, dst, dirs_exist_ok=True)
